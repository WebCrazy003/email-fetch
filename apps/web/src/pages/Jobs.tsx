...
import { ListFilter, Pause, Play, Plus, Square, Users } from 'lucide-react';
...
export function Jobs() {
  ...
  return <><PageHeader eyebrow="Background work" title="Jobs" description="Every saved-filter run appears here with its progress and results."
    ...
    <section className="panel"><div className="table-wrap"><table><thead><tr><th>Name</th><th>Status</th><th>Phase</th><th>Discovered</th><th>Inspected</th><th>Errors</th><th>Created</th></tr></thead>
      ...
      {!query.data!.items.length && <Empty detail="Run a saved filter to create your first job." />}<Pager page={page} total={query.data!.total} pageSize={25} onPage={setPage} /></section>
  </>;
}
...
export function JobDetail() {
  ...
  const controls = <div className="actions">{job.status === 'paused' ? <button className="button" onClick={() => action.mutate('resume')}><Play size={15} />Resume</button>
    ...
    {['running','queued','paused','rate_limited'].includes(job.status) && <button className="button danger" onClick={() => action.mutate('cancel')}><Square size={14} />Cancel</button>}
    {job.saved_filter_id && <Link className="button" to="/filters"><ListFilter size={14} />Filter</Link>}
    {['completed','completed_with_errors','failed','cancelled'].includes(job.status) && <Link className="button" to={`/users?jobId=${job.id}`}><Users size={14} />Results</Link>}</div>;
  const metrics = [
  ...
  return <><PageHeader eyebrow="Job" title={job.name || `GitHub · ${job.id.slice(0, 8)}`} description={`Created ${new Date(job.created_at).toLocaleString()}`} action={controls} />
    ...
    <div className="detail-grid"><section className="panel"><div className="panel-head"><div><h2>Filter snapshot</h2><p>The saved filter used for this run</p></div></div><pre className="json">{JSON.stringify(job.filters_json, null, 2)}</pre></section>
